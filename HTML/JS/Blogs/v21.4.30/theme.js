...
function getVersionConfig() {
    if (window.cnblogsConfig.CnVersions = window.cnblogsConfig.GhVersions, window.cnblogsConfig.isVersionMapping) {
        ...
        $.ajax({ type: "get", url: i, dataType: "text", async: !1, success: function(i) { window.themeVersion = !!i && JSON.parse(i), window.themeVersion && e() } })
    ...
    function e() {
        var i = function i(e) {
            var s = "";
            window.themeVersion && $.each(window.themeVersion, function(i) { if (window.themeVersion[i][0] === e) return s = window.themeVersion[i][1], !1 });
            return "" === s ? e : i(s)
...
function init() {
    ...
    }), $("#blog-news").prepend(sidebarHtml), $("#m-nav-list").append(e), "" === window.cnblogsConfig.blogUser && (window.cnblogsConfig.blogUser = i), $.ajaxSetup({ cache: !0 }), $.getScript(getJsDelivrUrl("loading.js"), function() {
        window.cnblogsConfig.hook.beforeLoading(pageLoading), pageLoading.initRebound(), pageLoading.initSpinner(), pageLoading.spinner.init(pageLoading.spring, !0), $.getScript(getJsDelivrUrl("lib/jquery.mCustomScrollbar.min.js"), function() {
...
function getJsDelivrUrl(i, e) { return i = setFileNameMin(i, e), "https://cdn.jsdelivr.net/gh/" + window.cnblogsConfig.GhUserName + "/" + window.cnblogsConfig.GhRepositories + "@" + window.cnblogsConfig.GhVersions + "/" + (i || "") }
...
fetch('https://v1.hitokoto.cn/?c=a&c=b')
    ...
    .then(data => {
        ...
        const hitokoto_auth = document.getElementById('hitokoto_Author')
        hitokoto.href = 'https://hitokoto.cn/?uuid=' + data.uuid
        hitokoto.innerText = data.hitokoto
        hitokoto_auth.innerText = '——《' + data.from + '》——'
    })
    .catch(console.error)
